...
> Actualizar todas las películas que hayan sido protagonizadas por 'Steve Guttenberg'
> o 'Ian Ziering', asígnandole una calificación de 10.
> A continuación, generar una consulta que obtenga dichas películas con
> los campos **nombre** (renombre de nombre_pelicula), **actor** (renombre de nombre_actor),
> **director** (renombre de nombre_director), **calificacion** y **duracion**.
...
examples:
  -
    ...
    solution_dataset: |
      nombre|actor|director|calificacion|duracion
      Sharknado|Ian Ziering|Anthony C. Ferrante|10|90
      Lavalantula|Steve Guttenberg|Mike Mendez|10|92
YAML
ej6.language_id = language.id
ej6.guide_id = guide.id
ej6.hint = <<-MD
Solución
...
UPDATE pelicula SET
calificacion = 10
WHERE nombre_actor in ('Steve Guttenberg', 'Ian Ziering');

SELECT nombre_pelicula AS nombre, nombre_actor AS actor,
nombre_director AS director, calificacion, duracion
FROM pelicula
WHERE actor in ('Steve Guttenberg', 'Ian Ziering');
```
MD
ej6.extra = <<-SQL
CREATE TABLE actor(
